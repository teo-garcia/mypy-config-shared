...
PACKAGE = "teo_mypy_config_shared"
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="teo-mypy-consumer-") as temp:
        temp_root = Path(temp)
        ...
        config_executable = venv_dir / (
            "Scripts/teo-mypy-config-path.exe" if os.name == "nt" else "bin/teo-mypy-config-path"
        )
        ...
        run([mypy, "--config-file", str(config_path), "."], cwd=consumer_dir)
        ...
        bad_result = subprocess.run(  # noqa: S603
            [mypy, "--config-file", str(config_path), str(bad_fixture)],
            cwd=consumer_dir,
            check=False,
            capture_output=True,
            text=True,
        )
        if bad_result.returncode == 0 or "no-untyped-def" not in bad_result.stdout:
            raise RuntimeError("installed mypy config did not enforce strict mode")

    sys.stdout.write("mypy wheel consumer smoke ok\n")
